...
fn main() {
    ...
    if bot_script.exists() {
        ...
        let target_dir = out_dir
            .parent()
            .unwrap() // incremental ou deps
            .parent()
            .unwrap() // debug ou release
            .parent()
            .unwrap(); // target
        ...
        if let Err(e) = fs::create_dir_all(&resources_dir) {
            println!(
                "cargo:warning=Não foi possível criar diretório de recursos: {}",
                e
            );
        } else {
            ...
            if let Err(e) = fs::copy(&bot_script, &dest) {
                println!(
                    "cargo:warning=Não foi possível copiar whatsapp-bot.cjs: {}",
                    e
                );
            } else {
                println!(
                    "cargo:warning=whatsapp-bot.cjs copiado para: {}",
                    dest.display()
                );
            }
        }
        ...
        println!(
            "cargo:warning=whatsapp-bot.cjs não encontrado em: {}",
            bot_script.display()
        );
    }
}
